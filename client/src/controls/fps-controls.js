...
// TODO: convert to typescript completely
export const FirstPersonControls = function (
    camera,
    mass,
    playerHeight,
    doubleJump,
    ...
    scope.intersections = {
        ...
        rightStrafe: scope.raycasters.rightStrafe.intersectObjects(
            worldObjects,
        ),
        ...
        checkIntersections: function () {
            ...
            this.forward = scope.raycasters.forward.intersectObjects(
                worldObjects,
            );
            this.backward = scope.raycasters.backward.intersectObjects(
                worldObjects,
            );
    ...
    scope.movements = {
        ...
        lock: function () {
            let intersections = scope.intersections;
            for (let direction in intersections) {
    ...
    scope.updateControls = function () {
        ...
        let validDelta = isNaN(scope.delta) === false;
        if (validDelta) {
            // Velocities
            ...
            scope.mass =
                scope.isBelowObject === true ? 500 : scope.originalMass;
